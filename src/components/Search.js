...
const Search = () => {
  ...
  const userNotFound = useSelector(searchErrorSelect);
  ...
  const handleSubmitInput = (e) => {
    e.preventDefault();
    dispatch(loadSearchUsers(textInput));
  };
  ...
  return (
    <SearchStyled>
      <div className="title">
        <h1 onClick={handleClickLogo}>devfinder</h1>
      ...
      <SearchCardStyled>
        ...
        <form onSubmit={handleSubmitInput}>
          ...
          />
          {userNotFound && <p className="red-message">No results</p>}
...
const SearchStyled = styled.div`
  .title {
    ...
    .dark-mode {
      ...
      &:hover {
        h4 {
          color: ${(prop) => prop.theme.darkModeBtn};
        }
        svg {
          path {
            fill: ${(prop) => prop.theme.darkModeBtn};
          }
        }
      }
    }
